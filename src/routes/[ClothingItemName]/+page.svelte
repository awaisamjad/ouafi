		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-16 mx-auto max-md:px-2">
			<div class="img">
				<div class="img-box h-full max-lg:mx-auto">
					<img src={image} alt="Image of {name}" class="max-lg:mx-auto lg:ml-auto h-full" />
				</div>
			</div>
			<div
				...
				<div class="data w-full max-w-xl">
					...
					<div class="w-full pb-8 border-b border-gray-100 flex-wrap">
						<div class="grid grid-cols-3 min-[400px]:grid-cols-5 gap-3 max-w-md">
							<!-- TODO Get the available sizes from the json. then depending if it matches all sizes we display them with or without shading/cross -->
							...
							{#each available_sizes as size}
								<button
									...
									class:active={chosen_size === size}
									on:click={() => {
										chosen_size = chosen_size === size ? '' : size;
										console.log(chosen_size);
									}}
									disabled={false}>{size}</button
								>
							{/each}
						</div>
					</div>
